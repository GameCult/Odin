...
fn swarm_surgery_plan(
    ...
    let next_target = ["nightwing-muninn", "muninn", "starfire-muninn"]
        .iter()
        .copied()
        .find(|candidate| {
            targets.iter().any(|target| {
                target.enabled
                    && target.daemon_id == *candidate
                    && daemon_surgery_plan(target, updated_at).status
...
fn daemon_transport_profile(
    ...
    let (current_transport, state, cut_line) = match target.daemon_id.as_str() {
        ...
        "nightwing-muninn" => (
            ...
            "Nightwing Muninn now publishes muninn.cultnet-rudp-remote-telemetry-and-move-hid over CultNet/RUDP from the long-running serve process while Move HID and evidence remain local telemetry authority.",
        ),
        "vili" => (
    ...
    match target.daemon_id.as_str() {
        ...
        "starfire-muninn" | "nightwing-muninn" => {
            ...
            if target.daemon_id == "starfire-muninn" {
                ...
                current_mechanism =
                    "Nightwing Muninn now runs as a long-lived serve process with Move HID input, --idunn-rudp-health 10.77.0.2:17870, daemon id nightwing-muninn, and contract muninn.cultnet-rudp-remote-telemetry-and-move-hid. Live Idunn accepts that daemon-published health from 10.77.0.3 while the daemon continues to own the Move evidence stream and HID telemetry state."
                        .to_string();
                cut_line =
                    "Keep health-nightwing-muninn.cmd as fallback evidence only; the Nightwing serve process itself now owns muninn.cultnet-rudp-remote-telemetry-and-move-hid publication."
                        .to_string();
                steps = vec![
            ...
            if target.daemon_id == "nightwing-eve-dashboard" {
                ...
                current_mechanism =
                    "Nightwing Eve browser reference publishes nightwing.cultnet-rudp-browser-reference-health over CultNet/RUDP from the Mimir.EveBrowserReference service process, and the runtime now writes a daemon-owned boundary store at /var/lib/gamecult/eve-browser-reference/cultcache/eve-browser-reference.service.cc."
                        .to_string();
                cut_line =
                    "Keep the service probe as fallback only until Nightwing projections and Odin consume the browser reference CultCache witness directly instead of treating the service probe as primary truth."
                        .to_string();
                steps = vec![
...
mod tests {
    ...
    fn swarm_surgery_plan_names_muninn_after_voidbot_cut() {
        ...
        assert_eq!(plan.next_target, "nightwing-muninn");
        assert!(plan.current_phase.contains("Raven and Starfire Muninn now publish"));
        assert!(
            plan.cut_line
        ...
        assert!(plan.cut_line.contains("Linux body"));
        ...
        assert!(plan.cut_line.contains("Nightwing Muninn still needs the same provider-store witness cut"));
        ...
        assert_eq!(raven_plan.status, "partial-rudp-health-and-provider-store-live");
        assert!(
            raven_plan
                .current_mechanism
        ...
        assert!(raven_plan.current_mechanism.contains("C:\\Meta\\Odin\\state\\muninn.telemetry.cc"));
        assert!(
            raven_plan
                .cut_line
                .contains("fallback evidence only")
        );
        ...
        assert_eq!(starfire_plan.status, "partial-rudp-health-and-provider-store-live");
        assert!(starfire_plan.current_mechanism.contains("127.0.0.1:17870"));
        assert!(starfire_plan.current_mechanism.contains("archives a corrupt CultCache store"));
        assert!(starfire_plan.current_mechanism.contains("C:\\Meta\\Odin\\state\\starfire.muninn.telemetry.cc"));
        ...
        assert_eq!(nightwing_plan.status, "partial-rudp-health-live");
        ...
        assert!(nightwing_plan.steps.iter().any(
            |step| step.contains("restart-nightwing-muninn.ps1")
        ...
        assert_eq!(heimdall_plan.status, "partial-rudp-health-and-provider-store-live");
        assert!(
            heimdall_plan
                .current_mechanism
    ...
    fn heimdall_transport_profile_marks_boundary_store_live() {
        ...
        assert!(profile.cut_line.contains("/srv/heimdall/cultcache/heimdall.service.cc"));
    }
    ...
    fn muninn_transport_profiles_mark_live_rudp_health() {
        ...
        assert_eq!(raven_profile.state, "partial-rudp-health-and-provider-store-live");
        assert!(raven_profile.current_transport.contains("daemon-owned-cultcache-telemetry-store"));
        assert!(raven_profile.current_transport.contains("background-only hidden task launch"));
        assert!(raven_profile.cut_line.contains("C:\\Meta\\Odin\\state\\muninn.telemetry.cc"));

        let starfire_profile = daemon_transport_profile(&starfire, "unix:100");
        assert_eq!(starfire_profile.state, "partial-rudp-health-and-provider-store-live");
        assert!(starfire_profile.current_transport.contains("daemon-owned-cultcache-telemetry-store"));
        assert!(starfire_profile.cut_line.contains("Quest ADB availability"));
        assert!(starfire_profile.cut_line.contains("C:\\Meta\\Odin\\state\\starfire.muninn.telemetry.cc"));

        let nightwing_profile = daemon_transport_profile(&nightwing, "unix:100");
        assert_eq!(nightwing_profile.state, "partial-rudp-health-live");
        assert!(nightwing_profile.cut_line.contains("Move HID and evidence"));
    }
