...
use cultcache_rs::{
    CacheBackingStore, CultCacheEnvelope, CultCacheExpectedEnvelope,
...
pub fn enroll_service_identity_at<P: ServiceIdentityProfile>(
    ...
    let backing = SingleFileMessagePackBackingStore::new(path);
    let expected = CultCacheExpectedEnvelope {
        r#type: P::PRIVATE_TYPE.into(),
        key: P::PRIVATE_KEY.into(),
        current: None,
    };
    if !backing.compare_exchange(&[expected], &[envelope])? {
        bail!("service identity enrollment lost atomic create race; refusing existing state");
    }
